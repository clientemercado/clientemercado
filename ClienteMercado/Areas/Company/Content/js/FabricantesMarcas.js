$(document).ready(function () {
    ...
    $("#btn-gravar").click(function (e) {
        ...
        if (qtdCamposVazios == 0) {
            ...
            //Obtendo o valores
            obj['descricao_EmpresaFabricantesMarcas'] = $("#inNomeFabricante").val();

            $.ajax({
                type: "POST",
                url: "/FabricantesMarcas/GravarRegistro",
                traditional: true,
                async: false,
                dataType: "json",
                ...
                success: function (data) {
                    ...
                    if (data.status == "ok") {
                        ...
                        window.location.href = "/Company/FabricantesMarcas/AlterarDados?id=" + idRegistroGerado;
                    }
                    else {
    ...
    $("#btn-alterar").click(function (e) {
        ...
        if (qtdCamposVazios == 0) {
            ...
            $.ajax({
                type: "POST",
                url: "/FabricantesMarcas/AtualizarRegistro",
                traditional: true,
                async: false,
                dataType: "json",
                contentType: "application/json",
    ...
    $(document).on("click", "#btn-pesquisar", function () {
        ...
        if ($('#inPesquisar').val() != "") {
            $('#gridFabricantesMarcas').bootgrid('reload');
        }
        else {
